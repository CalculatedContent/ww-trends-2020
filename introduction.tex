...
and so on.
A less well-known problem, but one that is increasingly important (in particular in industrial-scale ML---where the \emph{users} of models are not the \emph{developers} of the models) is that one may have access to neither the training data nor the testing data.
...
It is of great practical interest to have metrics to evaluate the quality of a ML model---in the absence of training/testing data and without any detailed knowledge of the training/testing process.  
...
\noindent
We emphasize that our goal is a practical theory to predict trends in the quality of state-of-the-art NN models, i.e., not to make a statement about every publicly-available model.
We have examined hundreds of models, and are exceptions to the general trends we state.
...
\footnote{\url{https://github.com/CalculatedContent/kdd2020} \michael{DIFFERENT NAME?}}
\michael{LINK TO: Google Colab, IS THIS THAT REPO.}
\michael{FIX THIS PAR.}
