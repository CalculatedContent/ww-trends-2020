...
\begin{abstract}
Understanding state-of-the-art overparameterized neural networks requires detailed empirical studies as well as the development of predictive theory in light of those studies. 
Building on the theory of Heavy-Tailed Self-Regularization, we show that correlations in the weight matrices of state-of-the-art overparameterized neural networks are well-described by heavy-tailed random matrix theory.
Using the publicly-available WeightWatcher tool, we perform spectral analylsis on these weight matrices, and we show that fitted truncated power law exponents are strongly predictive of model quality, for publicly-available state-of-the-art pre-trained models, even in the absence of training or testing data.
We also show that these metrics can also be used to identify a Simpson's paradox in the data from a recent contest aimed at developing metrics to predice generalization in deep learning.
\end{abstract}


\section{Introduction}

\michael{Here is Charles core dump of intro; still to refine and incorporate with rest of text.}
...
%%Empirical results clearly indicate that the empirical spectral density (ESD) of DNN layer matrices displays signatures of traditionally-regularized statistical models, even in the absence of exogenously specifying traditional forms of explicit regularization, such as dropout or weight norm~constraints.
%%
Building on these empirical results, 
%%and using relatively recent results in RMT, most notably its extension to Universality classes of heavy-tailed matrices, 
