...
In contrast, PL-based metrics, including the Log $\alpha$-Norm metric ($\log\Vert\mathbf{W}\Vert_{\alpha}^{\alpha}$) and the Weighted Alpha metric ($\hat\alpha =\alpha\log\lambda_{max} $) display consistent behavior, even on poorly trained models.
...
We see that the average $\alpha$ decreases with increasing model size, although the differences are less noticeable between the differing good-better-best GTP2 models than between the good-versus-bad GPT and GPT2-small models.
...
\begin{figure}[htb]
    ...
    \subfigure[PL exponent ($\alpha$)]{
        \includegraphics[width=5.5cm]{img/GPT2xxx_fnl_alpha_hist.png}
        \label{fig:gpt2-alpha-hist}
    }
    \quad
    \subfigure[Log Alpha Norm]{
        \includegraphics[width=5.5cm]{img/GPT2xxx_fnl_logpnorm_hist.png}
