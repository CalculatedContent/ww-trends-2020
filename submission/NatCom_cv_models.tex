...
\charles{The Log Spectral Norm metric correlates very well with small, simpler data sets like the VGG and DenseNet architectures, however, when moving to the larger and more complex ResNet series, the PL-based metrics Log $\alpha$-Norm ($\log\Vert\mathbf{W}\Vert_{\alpha}^{\alpha}$) 
and  Weighted Alpha ($\hat\alpha =\alpha\log\lambda_{max} $) metrics perform better across all measures.}
