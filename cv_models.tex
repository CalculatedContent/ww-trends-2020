...
In this section, we examine quality metrics described in Section~\ref{sxn:methods} for several Computer Vision (CV) model architecture series.
...
We use our quality metrics to perform \emph{coarse model analysis}, comparing and contrasting the three model series (or four, if ResNet-1K is included), permitting us to predict trends in model quality. 
We also use perform a more \emph{fine layer analysis} as a function of depth for these models, providing novel insights among the VGG, ResNet, and DenseNet architectures. 

\paragraph{Coarse Analysis: Quality Metrics versus Reported Test Accuracies.}

We have examined the performance of the four quality metrics (Frobenius norm, Spectral norm, Weighted Alpha, and $\alpha$-Norm) applied to each of the VGG, ResNet, ResNet-1K, and DenseNet series.
...
Here, the Log $\alpha$-Norm is much better thatn the Log Frobenius/Spectral norm metrics (although, as Table~\ref{table:cv-models} shows, it is actually slightly worse than the Weighted Alpha metric).
...
These model series are all well-trodden, and our results indicate that norm-based metrics and PL-based metrics can both distinguish ``good-better-best,'' with PL-based metrics performing somewhat better. 
...
To show that it is not just a variation and that it does capture something different, consider the following example, which involves DN model compressed/distillation~\cite{CWZZ17_TR}.
...
The results of Figure~\ref{fig:vgg-metrics} and Figure~\ref{fig:cv2-accuracy} might suggest that the baseline Spectral Norm should be \emph{smaller} than those of the layers in the fine-tuned model.
